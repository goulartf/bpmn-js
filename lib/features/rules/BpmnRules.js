...
function hasBoundaryEventAttached(element) {
  return find(element.attachers, function(attacher) {
...
function isReceiveTaskWithoutBoundary(element) {
  return (
    is(element, 'bpmn:ReceiveTask') &&
    !hasBoundaryEventAttached(element)
  );
}

function isEventBasedTarget(element) {
  return (
    isReceiveTaskWithoutBoundary(element) || (
      is(element, 'bpmn:IntermediateCatchEvent') && (
...
function canAttach(elements, target, source, position) {
  ...
  // do not attach after event based gateways
  console.log(target);
  console.log(getBusinessObject(target.incoming[0]));
